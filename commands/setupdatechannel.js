...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('업데이트채널설정')
        .setDescription('업데이트 채널을 설정합니다.')
        .addChannelOption(option =>
            option.setName('channel')
                .setDescription('업데이트 채널로 설정할 텍스트 채널을 선택해주세요.')
                .setRequired(true)),
    async execute(interaction) {
        ...
        // 채널 ID 저장
        channelConfig.updateChannelId = channel.id;

        // 파일에 저장
        fs.writeFileSync(channelConfigPath, JSON.stringify(channelConfig, null, 2));

        // 응답
        console.log(`업데이트 채널이 <#${channel.id}>로 설정되었습니다.`);
        return interaction.reply({ content: `✅ 업데이트 채널이 <#${channel.id}>로 설정되었습니다.`, ephemeral: true });
    },
};
